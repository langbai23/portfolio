...
async function GET() {
  ...
  const subtitle = latest ? `Latest: ${latest.year} · ${latest.id === "after" ? "After They're Gone" : latest.id === "traveler" ? "Traveler" : latest.id === "overtale" ? "Overtale" : latest.id === "cami" ? "CAMI" : "Picnic Panic!"}` : "Game design portfolio";
  const markup = html(`
    <div
      style="
        width: 100%;
